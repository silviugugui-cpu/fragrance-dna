...
export default async function StudioFamiliesPage() {
  try {
    const data = await loadBuilderControlCenterData();
    return <StudioBuilderControlCenter data={data} workspaceLabel="Families" initialSection="review" />;
  } catch (error) {
    const message =
